/*Exercise 1. makeCounter below is a decorator function which creates and returns a function that increments a counter.
...
//a. tests print #4, #3, #2, #1 because delayMsg doesn't use setTimeout and runs first, then the others according to their delays
//b.
const delayMsg = (msg) => console.log(`This message will be printed after a delay:
${msg}`);
...
See https://www.techtarget.com/whatis/definition/debouncing It's often used to handle fast-firing
scrolling events in a browser, or to prevent multiple server requests being initiated if a user clicks repeatedly on a button.
...
a) Use the example multiply function to test it with, as above, and assume that all delayed functions will take two parameters
...
/*Exercise 7. In JavaScript, the toString method is used to convert an object to a string representation.
...
/*Exercise 8. The following DigitalClock class uses an interval to print the time every second once
...
/*Exercise 9. We can delay execution of a function using setTimeout, where we need to provide both
...
/*Exercise 10. Fetch is a browser-based function to send a request and receive a response from a server,
